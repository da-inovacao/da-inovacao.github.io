    ...
    if (events !== undefined && events.length > 0) {
      return events.map((e, i) => (
        <div className='card event mb-4' key={i}>
          <div className='row'>
            <div
              className='col-2 bg-info rounded-start text-light py-2'
              style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}
            >
              <div className='row text-center'>
                <span className='small'>Dia</span>
                <span className='font-weight-bold small'>{getDay(e.created_at)}</span>
              </div>
            </div>
            <div className='col p-2'>
              ...
              <div className='row'>
                <span className='card-text small align-items-center'>
                  {getMonth(e.created_at)} {getYear(e.created_at)}
                </span>
              </div>
            </div>
          </div>
